  ...
  useEffect(() => {
    const fetchFMEA = async () => {
      try {
        ...
        const response = await fetch(`/api/fmea/${id}`);
        
        if (!response.ok) {
          throw new Error('Failed to fetch FMEA');
        }
        
        const fmea = await response.json();
        setFormData(fmea);
      } catch (error) {
        console.error('Error fetching FMEA:', error);
        toast({
          title: "Error",
          description: "Failed to load FMEA data",
          variant: "destructive"
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to update FMEA');
      }

      toast({
        title: "FMEA Updated",
        description: `FMEA ${fmeaData.fmeaNumber} has been updated successfully.`,
      });
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : 'Failed to update FMEA',
        variant: "destructive"
      });
    } finally {
      setLoading(false);
    ...
    <div className="pt-20 px-4">
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">Edit FMEA Analysis</h1>
        </div>
