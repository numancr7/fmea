...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    if (!token) {
      ...
      toast({ title: 'Error', description: 'No reset token provided.' });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      toast({ title: 'Error', description: 'Reset token is missing. Please try again from the forgot password link.' });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({ title: 'Error', description: data.message || 'An error occurred while resetting your password.' });
      }
    } catch {
